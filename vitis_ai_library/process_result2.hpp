...
vector<string> name = { "person", "bicycle", "car"};

static cv::Mat process_result(cv::Mat &image,
                              const vitis::ai::YOLOv3Result &result,
                              bool is_jpeg) {
  ofstream ofs( "det_result.txt" );
  if (!ofs) {
	  cout << "Cannot open det_result.txt file" << endl;
	  return image;
  }
  for (const auto bbox : result.bboxes) {
    ...
    //                      << "\n";
    ofs << name[label] << " " << confidence << " " << round(xmin) << " " << round(ymin) << " " << round(xmax)
	               << " " << round(ymax) << "\n";

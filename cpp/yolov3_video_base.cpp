                            ...
                            float input_scale)
{
    ...
    // setInput_end_time = chrono::system_clock::now();
    // cout << "\nsetInputImageForYOLO time= " << std::chrono::duration_cast<std::chrono::milliseconds>(setInput_end_time - setInput_start_time).count()
    //      << " [mS]" << endl;
}

Mat runYOLO(vart::Runner *runner, const Mat &frame, int index)
{
    ...
    */
    ...
    // original image is resized to 416x416.

    // setInputImageForYOLO(frame, imageInputs, input_scale);
    // setInputPointer(frame, imageInputs, input_scale);
    setInputPointer_OpenCV(frame, imageInputs, input_scale); // OpenCV version
    ...
    logger.logRow("pre_process", {index, pre_process_start, pre_process_dur});
    ...
    logger.logRow("exec_async", {index, dpu_start, dpu_dur});
    ...
    logger.logRow("post_process", {index, post_process_start, post_process_dur});
...
int main(const int argc, const char **argv)
{
    ...
    */
    ...
    while (index < maxFrame)
    {
        ...
        Mat frame = runYOLO(runner.get(), img, index);
        ...
        logger.logRow("readFrame", {index, read_start, read_dur});

        auto display_dur = duration_cast<microseconds>(t3 - t2).count();
        auto display_start = (duration_cast<microseconds>(t2 - program_start)).count();

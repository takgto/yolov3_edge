...
bool write_binary(const char* filepath, std::vector<float>& vec){
...
void get_output(int8_t* dpuOut, int sizeOut, float scale, int oc, int oh, int ow, vector<float>& result) {
    ...
    for(int a = 0; a < oc; ++a){
		...
		// (oc, oh, ow) --* (oh, ow, oc) * scale
            }
        }
    }
} 
// Not used My6-2023 by TG
void scaling_cast(const int8_t* dpuOut, const float& scale, vector<float>& result){
    size_t sizeOut = result.size();
    for (int i = 0; i < sizeOut; i++) {
	    result[i] = (static_cast<float>(dpuOut[i]) * scale);
    }
}
